...
public class SyncCrypto {
    ...
    private lazy var browserifyJS: BrowserifiedJS? = {
        return BrowserifiedJS(browserifiedScript: BraveCryptoJSFunctions())
    }()
    ...
    public func passphrase(fromBytes bytes: [Int]) -> JSResult<[String]> {
        ...
        guard let browserifyJS = browserifyJS else { return .failure(JSError.unknownError) }
        guard let bytesUInt8Array = browserifyJS.context.arrayToUint8Array(values: bytes) else {
            return .failure(JSError.unknownError)
        }
        
        do {
            let result = try browserifyJS.callFunction(
                withName: BraveCryptoJSFunctions.Names.fromBytesOrHex, arguments: [bytesUInt8Array])
            
            let words = result.toString().components(separatedBy: " ")
            return words.count == bipWordLength ? .success(words) : .failure(JSError.unknownError)
        } catch {
            log.error(error)
    ...
    public func bytes(fromPassphrase passphrase: [String]) -> JSResult<[Int]> {
        ...
        do {
            let result = try browserifyJS.callFunction(withName: BraveCryptoJSFunctions.Names.toBytes32,
                                                       arguments: [wordsWithSpaces])
            
            let bytes = result.toString().components(separatedBy: ",").compactMap { Int($0) }
            return bytes.count == 32 ? .success(bytes) : .failure(JSError.unknownError)
        } catch {
            log.error(error)
            return .failure(error)
        }
